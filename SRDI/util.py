...
def data_processing(input, missingratio, target_dim,test_idx_subset = None): # 我的mask
    batch = {}
    K = input.shape[1]
    temp = True
    for indx in range(K):
        if torch.all(input[:,indx,:]) == 0:
            temp = False
    
    if temp:
        mask_num = missingratio * target_dim  # 被随机抹掉的个数
        mask_num = int(mask_num)
        seed = torch.randint(0, target_dim, (mask_num,)) #被mask掉对应的索引
        mask = torch.ones_like(input)
        gt_mask = mask.clone()
        for i in range(len(seed)):
            id = seed[i]
            gt_mask[:, id, :] = 0.0
        batch["observed_data"] = input 
        batch["observed_mask"] = mask
        # diffusion 二次mask

        all_integers = torch.arange(0, target_dim)
        mask_z = torch.isin(all_integers, seed)
        other_integers = all_integers[~mask_z]
        for i in range(int(len(other_integers)*0.1)):
            idx = other_integers[i]
            gt_mask[:,idx,:] = 0.0

        batch["gt_mask"] = gt_mask
        timepoints = torch.arange(0,input.shape[2])
        timepoints = timepoints.unsqueeze(0)
        timepoints = timepoints.repeat(input.shape[0],1)
        batch["timepoints"] = timepoints

    else:#说明是子集
        batch["observed_data"] = input
        mask = torch.ones_like(input, dtype=torch.float)
        mask[input == 0] = 0
        batch["observed_mask"] = mask
        x = len(test_idx_subset)

        gt_mask = mask.clone()
        num = int(x*0.1)
        for i in range(num):
            idx = test_idx_subset[i]
            gt_mask[:,idx,:] = 0.0
